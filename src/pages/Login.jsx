  ...
  const loginUser = async (e) => {
    ...
    try {
      ...
      if (response.data.AdminNotFound) {
        setErrorAdmin({
          ...errorAdmin, message: response.data.AdminNotFound, error: true
        })
      }
      if (response.data.PasswordIsNotCorrect) {
        setErrorPass({
          ...errorPass, message: response.data.PasswordIsNotCorrect, error: true
        })
      }
      if (response.data.token) {
        localStorage.setItem('token', JSON.stringify(response.data.token))
        localStorage.setItem('adminInfo', JSON.stringify({ name: response.data.data.name, email: response.data.data.email }))
        dispatch(checkLogin('s'))
        setErrorAdmin({
          ...errorAdmin, message: "", error: false
        })
        setErrorPass({
          ...errorPass, message: "", error: false
        })
        navigate('/')
      }
    } catch (error) {
